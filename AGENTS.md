...
This codebase uses module-level API interfaces and dependency injection as its abstraction boundary.

Concrete implementation classes are already hidden behind module interfaces. Therefore, do **not** introduce an additional PImpl / implementation-object layer inside concrete classes.

Never create patterns such as:
...
Do not create `Impl`, `Implementation`, `Private`, `Data`, or similar classes whose purpose is merely to contain the private implementation of another class.

### Preferred design

Concrete classes should contain their actual state directly:
...
Put non-trivial function definitions in the corresponding `.cpp` file.

The architectural abstraction boundary belongs at the module interface:
...
Other modules depend on `IWorld`, not on the concrete `World` class.

### Design rule

Do not add abstraction or indirection inside a concrete implementation class merely to hide its implementation.

The module API already provides implementation hiding.

Prefer:

**module interface → concrete class**

not:

**module interface → concrete class → Impl class**

Only use PImpl if I explicitly request it for a specific class and there is a concrete ABI or binary-compatibility requirement.
...
- Do not create proxy or umbrella headers that only re-export other headers. Consumers include each concrete header they
  use directly.
